...
class SubMessage(object):
    ...
    def apply_transpose(self, transpose_dict):
        ...
        for char in self.message_text:
            if char.isalpha():
                transmessage += transpose_dict[char]
            else:
                transmessage += char
        return transmessage
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for per in all_permutations:
            ...
            transed=self.apply_transpose(transpose_dict)
            splited = transed.split()
            max_new = 0
            for word in splited:
                if is_word(self.valid_words,word):
                    max_new += 1
            if max_new >= max_old:
                decrypt_message = transed
                max_old=max_new    
...
if __name__ == '__main__':
    ...
    enc_message = EncryptedSubMessage('Yoi sey somuthang')
    print("Decrypted message:", enc_message.decrypt_message())